...
export function PublicWindow({ onManage, onLogin, onOrders }: { onManage: () => void; onLogin: () => void; onOrders: () => void }) {
    ...
    <main className="public-window">
      ...
      <section className="hero" id="top">
        <div className="hero-copy">
          ...
          <div className="hero-buttons"><button onClick={() => document.getElementById('route')?.scrollIntoView({behavior:'smooth'})}>规划我的南坡之行</button></div>
      ...
      <section className="quick-window" aria-label="南坡信息概览">
        <a href="#route"><span className="quick-no">01</span><div><small>HOW TO ARRIVE</small><b>怎么来南坡</b></div></a>
        <a href="#stay"><span className="quick-no">02</span><div><small>STAY IN VILLAGE</small><b>住进山居院落</b></div></a>
        <a href="#goods"><span className="quick-no">03</span><div><small>LOCAL HARVEST</small><b>把山野带回家</b></div></a>
        <a href="#nearby"><span className="quick-no">04</span><div><small>FEATURED DAY TRIPS</small><b>特色周边游</b></div></a>
        <a href="#experience"><span className="quick-no">05</span><div><small>PLAY &amp; HARVEST</small><b>游玩采摘</b></div></a>
      </section>
...
function Pagination({ page, total, pageSize, onChange, label, dark = false }: { page: number; total: number; pageSize: number; onChange: (page: number) => void; label: string; dark?: boolean }) {
  ...
  return <nav className={`pagination ${dark ? 'dark' : ''}`} aria-label={`${label}分页`}><span>共 {total} 项</span><div><button disabled={page<=1} onClick={()=>onChange(page-1)} aria-label={`上一页${label}`}>上一页</button>{Array.from({length:pages},(_,index)=>index+1).map(item=><button key={item} className={page===item?'active':''} onClick={()=>onChange(item)} aria-current={page===item?'page':undefined}>{String(item).padStart(2,'0')}</button>)}<button disabled={page>=pages} onClick={()=>onChange(page+1)} aria-label={`下一页${label}`}>下一页</button></div><small>{String(page).padStart(2,'0')} / {String(pages).padStart(2,'0')}</small></nav>;
}
    ...
    {plan ? <div className="plan-studio">
      <div className="plan-aside">
        ...
        <div className="plan-tabs">
          {(Object.entries(plans) as [NearbyPlanId, NearbyPlan][]).map(([id, item]) => <button key={id} className={planId === id ? 'active' : ''} onClick={() => setPlanId(id)}><span>{item.days}</span><b>{item.name}</b><small>{item.fit}</small></button>)}
        </div>
      </div>
...
const paymentQrCodes = [
  { name: '支付宝', image: '/images/payment-alipay.png', fileName: '南坡-支付宝收款码.png', tone: 'alipay' },
  { name: '微信支付', image: '/images/payment-wechat.png', fileName: '南坡-微信收款码.png', tone: 'wechat' },
];
